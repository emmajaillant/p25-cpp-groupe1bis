...
class Edge {
    ...
    friend class Vertex;
private:
    class Vertex* direction; 
    double poids;

    Edge(Vertex* dir, double p) : direction(dir), poids(p) {}
};
...
class Graph {
private:
    std::vector<Vertex*> matrice;
    std::unordered_map<std::string, int> index; //ici j'ai utilisé l'ia pour la synthaxe(unordered map) de comment pouvoir mettre differents arguments dans des cases
    std::vector<std::vector<double>> adjacente;

   
    void dfs_recursive(Vertex* v, std::set<std::string>& visite) {
        visite.insert(v->name);
        std::cout << v->name << " ";

        for (Edge* e : v->aretes) {
            if (visite.find(e->dir->name) == visite.end()) {
                dfs_recursive(e->dir, visite);
            }
        }
    }
...
public:
    ...
    void add_edge(const std::string& begin, const std::string& end, double value) {
        if (index.find(begin) == index.end()) {
            index[begin] = matrice.size();
            matrice.push_back(new Vertex(begin));
        }
        if (index.find(end) == index.end()) {
            index[end] = matrice.size();
            matrice.push_back(new Vertex(end));
        }

        Vertex* v_begin = matrice[index[begin]];
        Vertex* v_end = matrice[index[end]];
        
        v_debut->aretes.push_back(new Edge(v_end, value));
    }

    void dfs() {
        std::set<std::string> visite;
        std::cout << "Parcours DFS : ";
        for (Vertex* v : matrice) {
            if (visite.find(v->name) == visite.end()) {
                dfs_recursive(v, visite);
            }
        }
        std::cout << std::endl;
    }
    ...
    void matriceadj() {
        ...
        adjacente.assign(n, std::vector<double>(n, INF));
        ...
        for (int i = 0; i < n; ++i) {
            for (Edge* e : matrice[i]->aretes) {
                int j = index[e->dir->name];
                adjacente[i][j] = e->poids;
            }
        }
    }
    ...
    void floyd_warshall() {
        ...
        for (int k = 0; k < n; ++k) {
            for (int i = 0; i < n; ++i) {
                for (int j = 0; j < n; ++j) {
                    if (adjacence[i][k] + adjacence[k][j] < adjacence[i][j]) {
                        adjacence[i][j] = adjacence[i][k] + adjacence[k][j];
                    }
                }
            }
        }
